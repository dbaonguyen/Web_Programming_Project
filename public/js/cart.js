function addToCart(event) {
    ...
    if (productElement.querySelector('.product-img-2')) {
        ...
        productName = productElement.querySelector('.product-content-2 h4 a').textContent.trim();
        productPrice = productElement.querySelector('.product-content-2 span').textContent.trim();
    } 
    // Check for the second HTML structure
        ...
        productName = productElement.querySelector('.product-content h4 a').textContent.trim();
        productPrice = productElement.querySelector('.product-content span').textContent.trim();
    } 
    else {
    ...
    var existingProduct = document.querySelector(`.cart-item[data-product-name="${productName}"]`);
    if (existingProduct) {
  ...
  var cartItems = document.querySelectorAll('.cart-item:not(.cart-item-template)');
  var cartData = [];
  ...
  var productsInCart = document.querySelectorAll('.cart-item:not(.cart-item-template)');
  if (productsInCart.length === 0) {
      document.querySelector('.empty-cart-message').style.display = '';
  } else {
      document.querySelector('.empty-cart-message').style.display = 'none';
  ...
  var cartCountElement = document.querySelector('.shop-count.book-count');
  var currentCount = document.querySelectorAll('.cart-item:not(.cart-item-template)').length;
  cartCountElement.textContent = currentCount;
  }
  ...
  function removeFromLocalStorage(productName) {
    var cartData = JSON.parse(localStorage.getItem('cartData') || '[]');
    var updatedCartData = cartData.filter(product => product.name !== productName);
  ...
  function updateCartTotals() {
    ...
    // Update on the page
    document.querySelector('.cart-page-total ul li:nth-child(1) span').textContent = subtotal.toFixed(2);
    document.querySelector('.cart-page-total ul li:nth-child(2) span').textContent = total.toFixed(2);
  }
  
  document.addEventListener("DOMContentLoaded", function() {
    loadCartToTable();
    // Event for Apply Coupon
    document.querySelector('input[name="apply_coupon"]').addEventListener('click', function(e) {
        e.preventDefault();
        updateCartTotals();
    });
  
    // Event for Update Cart
    document.querySelector('input[name="update_cart"]').addEventListener('click', function(e) {
        e.preventDefault();
        updateCartTotals();
    });
  });
  ...
  document.addEventListener("DOMContentLoaded", function() {
    // Event for Checkout
    document.querySelector('.cart-page-total a').addEventListener('click', function(e) {
        e.preventDefault(); // Prevent default link action
  
        // Clear all products from the cart (cart page)
        var cartPageItems = document.querySelectorAll('.table-content table tbody tr');
        cartPageItems.forEach(item => {
            item.remove();
        });
  
        // Clear all products from the mini cart
        var miniCartItems = document.querySelectorAll('.cart-item:not(.cart-item-template)');
        ...
        // Reset cart totals
        document.querySelector('.cart-page-total ul li:nth-child(1) span').textContent = '0';
        document.querySelector('.cart-page-total ul li:nth-child(2) span').textContent = '0';
  ...
  var addToCartButtons = document.querySelectorAll('.product-action .animate-top');
